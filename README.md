...
Atualmente, o controle financeiro sobre grandes volumes de processamento (Big Data) é reativo: o custo operacional só é conhecido após a execução completa dos processos, quando a fatura da nuvem é emitida. Isso gera um alto risco de overspending (gastos inesperados devido a queries ineficientes ou aumento de volume) e impede que a empresa aproveite variações de preços favoráveis em outros provedores de nuvem. 
